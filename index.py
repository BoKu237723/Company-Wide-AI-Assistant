...
class DepartmentAI:
    ...
    def load_department_data(self, department):
        ...
        if not filename:
            return f"Could not find data file for {department}"
        
        try:
            with open(filename, 'r') as f:
                return f.read()
        except Exception as e:
            return f"Error in loading {filename}: {e}"
    ...
    def get_available_departments(self):
        ...
        for department in self.departments:
            try:
                with open(self.department_files[department], 'r') as f:
                    available_departments.append(department)
            except Exception as e:
                print(f"Null in gettting available departments: {e}")
                continue
        return available_departments
    
def main():
    ai = DepartmentAI()
    print(" === Company Wide AI Assistant === ")
    ...
    while True:
        ...
        if department not in ai.departments:
            print(f"Invalid department, Could not find {department} in department list.")
            continue
