...
class DepartmentAI:
    ...
    def find_department_folders(self):
        ...
        try:
            ...
            main_folder = results.get('files', [])
            
            if not main_folder:
                print("❌ 'Company Reports' folder not found")
                return {}
            
            main_folder_id = main_folder[0]['id']
            print(f"✅ Found Company Reports folder")
            
            for department in self.departments:
                query = f"'{main_folder_id}' in parents and name='{department}' and mimeType='application/vnd.google-apps.folder' and trashed=false"
                results = self.drive_service.files().list(
                    q=query, 
                    spaces='drive', 
                    fields='files(id, name)'
